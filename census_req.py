...
# get these here: https://api.census.gov/data/2020/acs/acs5/variables.html
name = "NAME"
...
tract_vars_2020 = tuple([name, mean_inc, num_public_transit, population] + building_const)
tract_vars_2000 = tuple([name] + building_const)

c = Census(API_KEY)

# handle MSA-level data
msa_data = c.acs5.get(msa_vars, geo={"for": "metropolitan statistical area/micropolitan statistical area:*"}, year=2020)

with open('./data/msa_data.json', 'w') as f:
    json.dump(msa_data, f)

get_tract_data(c, tract_vars_2020, 2020)
